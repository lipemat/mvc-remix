<?php
/**
 * Adds a custom featured post widget to allow for custom post types and taxonomies
* Pretty Much Copies the Genesis Built in Widget
* @author mat lipe
* @since 1.22.13
* @param Must set the $post_type and $taxonomy_name to the correct
* 
* @TODO Add this to Core Framework
* 
*/
class CustomFeaturedPosts extends WP_Widget{
    /**
     * Set these values !!!!!!!!!
     */
    
    //TODO Turn these into dropdown selectable per Widget
    //TODO The Taxonomy should Populate automatically based on the selected Custom Post Type
    protected $post_type = 'project';
    protected $taxonomy_name = false;
    ...
    function __construct() {
        ...
        $title = ucwords(str_replace('_', '', $this ->post_type));

        $widget_ops = array (
                'classname'   => $this ->post_type. '-featuredpost',
                'description' => __( 'Displays featured posts from the '.$title.'  posts' ),
        );

        $control_ops = array (
                'id_base' => $this ->post_type. '-featured-post',
                'width'   => 505,
                'height'  => 350,
        );

        $this->WP_Widget( $this ->post_type. '-featured-post', __( 'Vimm '.$title. ' Posts'), $widget_ops , $control_ops );
    ...
    function widget( $args, $instance ) {
        ...
        //Add the taxonomy to the query if set
        if( $this ->taxonomy_name != false ){
...
?><p >
<label for="<?php echo $this->get_field_id( 'posts_cat' ); ?>" ><?php _e( 'Category', 'genesis' ); ?>:</label ><?php
