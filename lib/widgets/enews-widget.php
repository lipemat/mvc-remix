<?php
/**
 * Genesis eNews and Updates widget class. Totally customized to be better
 *
 * @category Genesis
 * @package Widgets
 *
 ...
 * @since 5.14.0
 */
class MVC_eNews_Updates extends WP_Widget {
    /**
    ...
    function __construct() {
        ...
        $widget_ops = array(
            ...
            'description' => __( 'Displays subscribe form for feedburner or other common email signup services', 'genesis-enews-extended' ),
        );

        parent::__construct( 'enews', __( 'Genesis Enews & Updates', 'genesis-enews-extended' ), $widget_ops );
    }
    ...
    function widget( array $args, array $instance ) {
        ...
        if (empty($instance['lname_text'])) {
            ...
            <form id="subscribe" action="<?php echo esc_attr( $instance['action'] ); ?>" method="post" <?php if ($instance['open_same_window'] == 0 ) : ?> target="_blank"<?php endif; ?> onsubmit="if ( subbox1.value == '<?php echo esc_js( $instance['fname_text'] ); ?>') { subbox1.value = ''; } if ( subbox2.value == '<?php echo esc_js( $instance['lname_text'] ); ?>') { subbox2.value = ''; }" name="<?php echo esc_attr( $instance['title'] ); ?>">
                ...
                <?php if ( ! empty($instance['fname-field'] ) ) : ?>
                    <p>
                        <input type="text" id="subbox1" class="enews-subbox" value="<?php echo esc_attr( $instance['fname_text'] ); ?>" onfocus="if ( this.value == '<?php echo esc_js( $instance['fname_text'] ); ?>') { this.value = ''; }" onblur="if ( this.value == '' ) { this.value = '<?php echo esc_js( $instance['fname_text'] ); ?>'; }" name="<?php echo esc_attr( $instance['fname-field'] ); ?>" />
                    </p>
                        
                <?php endif ?>
                
                <?php if ( ! empty($instance['lname-field'] ) ) : ?>
                    <p>
                        <input type="text" id="subbox2" class="enews-subbox" value="<?php echo esc_attr( $instance['lname_text'] ); ?>" onfocus="if ( this.value == '<?php echo esc_js( $instance['lname_text'] ); ?>') { this.value = ''; }" onblur="if ( this.value == '' ) { this.value = '<?php echo esc_js( $instance['lname_text'] ); ?>'; }" name="<?php echo esc_attr( $instance['lname-field'] ); ?>" />
                    </p>
                <?php endif ?>
                    <p>
