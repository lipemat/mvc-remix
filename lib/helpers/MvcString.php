<?php
        /**
         * String Helpers only available in Views
         * @since 8.6.13
         * @author Mat Lipe
...
class MvcString {
     ...
     * @param string [$moreLinkText] - the text for the read more link - (defaults to false)
     * @param array $args (
     *               strip_tags => true
     *              'strip_teaser' => false
    ...
    public function theContentLimit($maxChar, $moreLinkText = false, $args = array()) {
        ...
        if( $moreLinkText ){
            $content .= apply_filters('content-limit-read-more',
                            '<a class="read-more" href= '.
                                 apply_filters('the_permalink', get_permalink()).'>'. $moreLinkText.
                            '</a>', $post);
        }
        return $content;
   }
   ...
   public function postContentLimit($post, $maxChar, $moreLinkText = false, $args = array()){
        ...
        if( $moreLinkText ){
            $content .= apply_filters('content-limit-read-more',
                            '<a class="read-more" href= '.
                                 apply_filters('the_permalink', get_permalink($post->ID)).'>'. $moreLinkText.
                            '</a>', $post );
        }
        return $content;    
    ...
    * @param bool [$striptags] - to remove the html tags from the content (defaults to true)
    * @param array $args (
    *           'allowed_tags' => '<p><b><strong><em><i><u><del><span style="text-decoration: underline;">',
                'strip_tags' => true
    */     
   ...
   /**
     ...
     * * Our designers seem to think we can drag elements all over the page like Dreamweaver or something!.!
     * * Well, I'm here to tell you this is bullshit. - so instead of getting pissed, I wrote this.
     * 
    ...
    function wrapPipesAndDashes($output){
        $output = str_replace( 
                                    array('|','-'), 
                                    array( '<span class="pipe">|</span>','<span class="dashes">-</span>'),
                                    $output
                              );
    ...
    /**
     * Wraps the pipes with spans (had to do this because the dashes were breaking links
     * * Seems excessive right? Well think again. 
     * * Our designers seem to think we can drag elements all over the page like Dreamweaver or something!.!
     * * Well, I'm here to tell you this is bullshit. - so instead of getting pissed, I wrote this.
     * 
    ...
    * @example echo find_tag_content($s, 'body'), "\n";
    ...
    function find_between($s, $str1, $str2, $case_sensitive = false) {
       if($case_sensitive == false){
             ...
             // adds the length of the string to the start just in case the end is in the string
             $start += strlen($str1);
